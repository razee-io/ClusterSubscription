...
const createRemoteResources = async (razeeApi, apiKey, subscriptions, clusterId) => {
  ...
  try {
    return Promise.all(subscriptions.map(async sub => {
      ...
      let rendered;
      let backendService;
      let secretNameGit = 'satellite-config-gitops';
      if (sub.url) {
        rendered = Mustache.render(requestsTemplate, { url: url, secretName: secretName, namespace: NAMESPACE });
        backendService = 'generic';
      } else if (sub.remote) {
        const provider = sub.remote.remoteType;
        let repo;
        let branch;
        let filePath;
        sub.remote.parameters.map((param) => {
          if (param.key == 'repo') {
            repo = param.value;
            ...
            secretNameGit = param.value;
          }
        });
        rendered = Mustache.render(requestsTemplateGit, { repo: repo, provider: provider, branch: branch, filePath: filePath, secretName: secretName, secretNameGit: secretNameGit, namespace: NAMESPACE});
        backendService = 'git';
      }
      ...
      const secretJson = {
        ...
        'metadata': {
          ...
          'name': secretName,
          'annotations': {
            'deploy.razee.io/clustersubscription-secret': sub.subscriptionUuid,
            'deploy.razee.io/clusterid': clusterId
          },
          'labels': {
            'razee/watch-resource': 'lite'
