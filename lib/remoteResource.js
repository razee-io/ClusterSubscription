...
const createRemoteResources = async (razeeApi, apiKey, subscriptions, clusterId) => {
  ...
  try {
    return Promise.all(subscriptions.map(async sub => {
      ...
      await applyResource(RR_API_VERSION, 'RemoteResource', remoteResourceJson);
      await applyResource('v1', 'Secret', secretJson);
    }));
...
const applyResource = async (apiVersion, kind, resourceJson) => {
