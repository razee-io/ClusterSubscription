...
const deleteRemoteResources = async (invalidResources) => {
  const krm = await kc.getKubeResourceMeta(API_VERSION, KIND, 'update');
  const selfLinks = invalidResources.map((resource) => krm.uri({ name: resource.metadata.name, namespace: resource.metadata.namespace }));
