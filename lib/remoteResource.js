...
const resourceApiVersion = objectPath.get(resourceJson, 'metadata.apiVersion', RP_API_VERSION);
const resourceKind = objectPath.get(resourceJson, 'metadata.kind', '');
...
<<<<<<< HEAD
  ...
  const selfLinks2 = resources.map((resource) => krm_secret.uri({ name: `${resource.metadata.name}-secret`, namespace: resource.metadata.namespace, }));
  log.debug('Deleting', { selfLinks2 });
  await deleteResource(selfLinks2, krm);
=======
  await deleteResource(selfLinks, krm);
  ...
  log.debug('Deleting', { selfLinks });
  await deleteResource(selfLinks, krm);
>>>>>>> c4b7d3beda3019e1a6bfda34ac847ecc18305f16
};
